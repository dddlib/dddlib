namespace dddlib.Playground.Anemic
{
    ...
    public class Application
    {
        public void Demonstrate()
        {
            ...
            var registration = new Registration { Number = "BD02 XYZ" };
            var car = new Car { Registration = registration };
            repository.Save(car);

            // speed trap #1
            var sameCar = repository.Load(registration.Number);
            sameCar.MaxRecordedSpeed = car.MinRecordedSpeed = 67;
            repository.Save(sameCar); // unnecessary?

            // speed trap #2
            var stillSameCar = repository.Load(registration.Number);
            stillSameCar.MaxRecordedSpeed = 167;
